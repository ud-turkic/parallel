        ...
        if id_str.startswith("cairo-"):
            ...
            try:
                ...
                return (
                    ...
                    0,
                    num_part,
                )  # Add extra tuple element for string comparisons
...
def main():
    ...
    lang_group = parser.add_argument_group(
        "language options", "Specify which languages to include (at least one required)"
    )
    lang_group.add_argument(
        "--turkish", metavar="FILE", help="Path to Turkish treebank file"
    )
    lang_group.add_argument(
        "--azerbaijani", metavar="FILE", help="Path to Azerbaijani treebank file"
    )
    lang_group.add_argument(
        "--kyrgyz", metavar="FILE", help="Path to Kyrgyz treebank file"
    )
    lang_group.add_argument(
        "--uzbek", metavar="FILE", help="Path to Uzbek treebank file"
    )
    ...
    for sent_id in sorted_ids:
        ...
        if "Azerbaijani" in language_data:
            azerbaijani_text = (
                language_data["Azerbaijani"].get(sent_id, {}).get("text", "")
            )
